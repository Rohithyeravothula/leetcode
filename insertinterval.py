...
class Solution:
    def insert(self, intervals_format, newInterval_format):
        ...
        newInterval = [newInterval_format.start, newInterval_format.end]
        ...
        for single_iter in intervals:
            if not inserted and single_iter[1] > newInterval[0]:
                inserted = True
                stk.extend(compare_and_merge(single_iter, newInterval))
            else:
                if len(stk) == 0:
        ...
        if not inserted:
            top = stk.pop()
            order = [top, newInterval_format]
            order.sort()
            print(order)
            stk.extend(compare_and_merge(order[0], order[1]))
        ans = []
        for e in stk:
